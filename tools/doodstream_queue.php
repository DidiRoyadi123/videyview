...
/**
 * Doodstream Pending Queue Video Mover
 * 
 * Script ini berfungsi untuk memisahkan file video yang belum berhasil 
 * di-upload ke Doodstream ke dalam folder terpisah (`doodstream_pending`).
 * Berguna saat ingin upload bulk lewat Doodstream Desktop/Web uploader, 
...
$targetDir = __DIR__ . '/public/storage/doodstream_pending';
...
foreach ($files as $file) {
    ...
    if ($needsUpload) {
        ...
        if (rename($sourcePath, $targetPath)) {
            echo "[DIPINDAH] $fileName -> (Belum di Doodstream)\n";
            $movedCount++;
        } else {
...
echo "Total Video aman (Sudah di Doodstream) : $skippedCount\n";
echo "Lokasi Folder Pending: $targetDir\n";
echo "============================================\n";
echo "Catatan: Setelah selesai diupload manual, Anda bisa memindahkan ('Cut & Paste') kembali\nsemua file dari folder 'doodstream_pending' ke folder 'videos' agar terbaca normal di aplikasi lokal Anda.\n";
